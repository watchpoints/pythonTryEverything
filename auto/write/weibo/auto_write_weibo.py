...
class CMyWeiBo:
    ...
    def msg_up_load(self, page: Page, picture_path_list,habit_name:str, habit_detail:str):
        ...
        msg = habit_name + "\r\n"
        msg += habit_detail
        page.locator(".Form_input_2gtXx").fill(msg)
        time.sleep(5)
        # playwright codegen https://weibo.com/
        print("开始上传图片")
        with page.expect_file_chooser() as fc_info:
            #page.locator("css=.wbpro-iconbed.wbpro-iconbed-v2.woo-box-flex.woo-box-alignCenter\
            #              .woo-box-justifyCenter.undefined").click()
            page.get_by_title("图片").locator("i").click()

        file_chooser = fc_info.value
        file_chooser.set_files(picture_path_list)
        time.sleep(10)

        page.locator(".woo-button-main.woo-button-flat.woo-button-primary.woo-button-m.woo-button-round.Tool_btn_2Eane").click()
        print("发送")
        time.sleep(5)
...
def interface_auo_upload_zhihu_small():
    ...
    autoupload = CMyWeiBo(cookies_path, login_url, upload_picture_url,upload_mp4_url)
...
def interface_auo_upload_msg_weibo(file_path_list:str,habit_name :str,habit_detail :str):
    """
      对外调用接口
    """
    print("interface_auo_upload_zhihu_small")
    ...
    if sys == "Windows":
        cookies_path = r"D:\mp4\etc\weibo.json"
    elif sys == "Darwin":
        cookies_path = r"/Users/wangchuanyi/mp4/etc/weibo.json"
    else:
        cookies_path = r"/root/bin/zhihu_small.json"
    autoupload = CMyWeiBo(cookies_path, login_url, upload_picture_url,upload_mp4_url)
    with sync_playwright() as playwright:

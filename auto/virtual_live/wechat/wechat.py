...
class wechatlive():
    ...
    def listen_msg(self, page: Page):
        ...
        page.wait_for_selector("xpath=//*[@class='vue-recycle-scroller__item-wrapper']", state='attached')
        print("login is ok")
        last_data_id = 2
        last_barrage = None
        while True:  # 无限循环，伪监听
            ...
            if  chat_msgs_list.count() <= last_data_id:
                ...
                while  last_data_id < chat_msgs_list.count():
                    ...
                    user = item.locator("css=.message-username-desc").inner_text()
    ...
    def Connect(self):
        print("login")
        self.login_url = "https://channels.weixin.qq.com/login.html"
        self.room_id = "https://channels.weixin.qq.com/platform/live/liveBuild"
        # 说明：视频号 cookies很容易失效 第三天可能实现 todo
        if platform.system() == "Windows":
            self.cookies_path = r"./shipinhao.json"
        else:
            self.cookies_path = r"./shipinhao.json"
        # 打开
